...
%initialGuesses = [kdeg3 ec503 kobs]; 

%% read in experimental data  
simVal = readtable('intStagC.xlsx'); 
sim330 = simVal.yAS331;
sim330 = sim330'; 
%simTime = (0:16); %This will have to be changed to real values
simTime = [1 6:18 29];
simTime = simTime.*60;
...
% fun = @(x, t) oof(tspan, initialC, initialGuesses); 
% [B,resnorm,residual,exitflag,output,lambda,jacobian] = lsqcurvefit(fun, initialGuesses, simTime, ydata); 
...
%theta0 = rand(8,1); 
%lb = [0.001 100 1e-4 1.0 0 0 ]; 
%ub = [0.10 300 1e-3 2.0 200 200 200 3.0]; 
...
%% try multi start and global search 
% problem = createOptimProblem('lsqcurvefit', 'objective', @kinetics, 'x0', ..., 
%      theta0, 'xdata', t, 'ydata', c, 'lb', lb, 'ub', ub, 'options', options); 
%  gs = MultiStart; 
...
%times = linspace(simTime(1),simTime(end));
plot(simTime,c,'ko')
hold on
plot(time,MKATE2); hold on
%times,fun(x, times) ,'b-') %this part doesnt work either 
...
function C = kinetics(theta,t)
    ...
    function dC = DifEq(t,c)
        ...
        %kdeg3 = theta(1); %kdeg3 guess
        %ec503 = theta(2); %ec503 guess
        %kobs = theta(3); %kobs guess
